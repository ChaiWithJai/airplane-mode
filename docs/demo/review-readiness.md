...
| #5 Trajectory recorder | Local durable MVP; encryption still open | `/api/trajectory` builds a scrubbed `(s,a,r,s')` JSON tuple, gates the exact serialized payload with the same verifier, appends gate-clean records to local JSONL, and returns a count recovered from the store. The web loop records a trajectory only after Slack send succeeds, so preview/failed sends do not count as completed loops. No policy training. Not enclave-encrypted at rest yet. |
...
| #9 iOS/R1 | Simulator artifact only; hardware blocked | `shells/ios` Swift package proves choreography only; `docs/deprecations/deferred/ios-shell-scaffold.md` documents non-claims. Real mlx-swift/R1 measurement still requires physical device. |
