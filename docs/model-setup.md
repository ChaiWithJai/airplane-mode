...
For the native iPhone path, Bonsai ships **MLX** weights on `prism-ml/` for mlx-swift — that's the tamper-evident, radios-off shell tracked in [issue #9](https://github.com/ChaiWithJai/bonsai-phi-scrubber/issues/9). The GGUF + `llama-server` path here is the laptop edge node and the reproduction front door.
