...
| `mcp/` | M4 | llama-server HTTP (shared w/ CLI) | ephemeral/file | MCP tool-call | tool-result/Slack | "an agent can do it too" |
...
Build order: `cli` (M1) **before** `ios` (M3). `mcp` (M4) is a thin transport over the same core API the CLI already calls.
