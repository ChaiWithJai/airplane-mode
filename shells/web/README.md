...
./run.sh web                          # serves http://localhost:8099 and prints a LAN URL
```

- **On your Mac:** open `http://localhost:8099`.
- **On your phone (same Wi-Fi / hotspot):** open the printed `http://<laptop-ip>:8099`.

`POST /api/scrub {text}` → `{ scrubbed_text, redactions[], gate_pass, residual_count, record }`.
The explicit JSON contract and sample fixture live in `../../docs/contracts/`.
...
Same-Wi-Fi is the primary path: open the printed `http://<laptop-ip>:8099` on a phone
...
`http://172.20.10.x:8099` on the phone. No server restart is needed, since the server binds
