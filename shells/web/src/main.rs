...
use airplane_core::{scrub, InferenceProvider, InferenceRequest, Pack, Sampling, Span};
...
impl LlamaServer {
    fn chat(&self, system: &str, user: &str, schema: &Value, sampling: Sampling) -> Result<String> {
        ...
        let resp = ureq::post(&self.url).send_json(body)
            .map_err(|e| anyhow::anyhow!("llama-server failed ({e}); run ~/projects/bonsai/scripts/serve.sh"))?;
...
fn pseudonym(redactions: &[Span]) -> String {
    ...
    let person = redactions.iter().find(|s| s.entity == "PERSON").map(|s| s.text.as_str()).unwrap_or("client");
...
fn structure(model: &LlamaServer, scrubbed: &str) -> Value {
    ...
    let raw_rec = match model.chat(sys, scrubbed, &schema, Sampling { seed: 7, ..Sampling::model_card() }) {
...
fn looks_junk(s: &str) -> bool {
    s.is_empty() || s.contains('[') || s.contains(']') || s.contains(':')
        || s.split_whitespace().count() > 6 || s.len() > 48
}
fn sanitize_record(rec: &Value, scrubbed: &str) -> Value {
    let ground = scrubbed.to_lowercase();
    let themes: Vec<Value> = rec["themes"].as_array().cloned().unwrap_or_default().into_iter()
        .filter(|t| t.as_str().map(|s| !looks_junk(s)).unwrap_or(false))
        .take(3).collect();
    let commitments: Vec<Value> = rec["commitments"].as_array().cloned().unwrap_or_default().into_iter()
        .filter_map(|c| {
            ...
            let grounded = t.split_whitespace().any(|w| w.len() > 3 && ground.contains(&w.to_lowercase()));
            if t.is_empty() || t.contains('[') || t.len() > 56 || !grounded { None }
            else { Some(json!({"text": t, "status": "open"})) }
        })
        .take(2).collect();
...
fn handle_scrub(body: &str) -> Result<Value> {
    ...
    let redactions: Vec<Value> = res.redaction_map.iter()
...
fn local_ips() -> Vec<String> {
    ...
    std::process::Command::new("sh")
        ...
        .output()
        .ok()
        .map(|o| String::from_utf8_lossy(&o.stdout).split_whitespace().map(|s| s.to_string()).collect())
        .unwrap_or_default()
}
...
fn main() -> Result<()> {
    ...
    for mut req in server.incoming_requests() {
        ...
        match (method.as_str(), url.as_str()) {
            ("GET", "/") => {
                let html = std::fs::read_to_string(INDEX).unwrap_or_else(|_| "<h1>index.html missing</h1>".into());
                let _ = req.respond(tiny_http::Response::from_string(html).with_header(html_header));
            }
            ("GET", "/api/health") => {
                let _ = req.respond(tiny_http::Response::from_string(r#"{"ok":true}"#).with_header(json_header));
            }
            ("POST", "/api/scrub") => {
